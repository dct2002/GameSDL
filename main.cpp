...
// vẽ một text trên màn hình, bắt đầu từ điểm (x, y)
...
// vẽ một đường thẳng đứng (khi dx = 0, dy = 1) hoặc ngang (khi dx = 1, dy = 0)
...
//vẽ bảng với tất cả các khối và điểm
...
bool checkLose(int **&tab, int size) {
	...
	for (int i = 0; i < size; i++) {
		for (int j = 0; j < size - 1; j++) {
			if (tab[i][j] == tab[i][j + 1]) lose = 1;
		}
	}
	for (int i = 0; i < size - 1; i++) {
		for (int j = 0; j < size; j++) {
			if (tab[i][j] == tab[i + 1][j]) lose = 1;
		}
	}
...
//kiểm tra xem có động thái nào không
...
//di chuyển các điểm theo bất kỳ hướng nào
...
//tạo danh sách điểm cho kích thước bảng
...
int main(int argc, char **argv) {
	...
	while(!quit) {
		...
		//HIỂN THỊ BẮT ĐẦU ĐỂ CHỌN KÍCH THƯỚC TỪ 3 ĐẾN 10
		if (!getStart) {
			...
			sprintf(text, "Choose size of map: \030 - Bigger, \031 - Smaller Enter - START");
					...
					case SDL_KEYDOWN:
						if (event.key.keysym.sym == SDLK_ESCAPE) quit = 1;
							...
							while (!show_ranking) {
								...
								//kiểm tra xem có danh sách người chiến thắng không
		...
		//KIỂM TRA NẾU NGƯỜI CHƠI THUA VÀ HIỂN THỊ KẾT THÚC
	...
	//giải phóng tất cả các bề mặt và bộ nhớ
	for (int i = 0; i < size; ++i)
		delete[] tab[i];
	delete[] tab;
